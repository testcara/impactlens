...
- **Detection method**: Analyzes Git commit messages for AI assistance markers (see detection patterns below)
...
- **Detection method**: ImpactLens scans **all commit messages** in each PR for AI assistance markers (case-insensitive)
- **Supported patterns**: See [`impactlens/utils/pr_utils.py#L65-L79`](https://github.com/testcara/impactlens/blob/master/impactlens/utils/pr_utils.py#L65-L79) for the complete list of detection patterns
- **How it works**: A PR is marked as "Claude PR" if **at least one commit** contains any Claude pattern. Same for Cursor.
- **Note**: PRs can use **multiple tools** (counted in both "Claude PRs" and "Cursor PRs" if different commits use different tools)
- **Example**: A PR with 5 commits where 2 used Claude and 1 used Cursor → counted in both "Claude PRs" (1) and "Cursor PRs" (1)
- **Common patterns**: `Assisted-by: Claude`, `Co-authored-by: Claude`, `Code-assisted with Cursor AI` (auto-generated by Cursor), etc.
- **💡 Tip**: Use [Git hooks](../.githooks/README.md) to automatically add AI trailers to commits for consistent tracking
- **Why it matters**: Tracks which AI tools are most popular and identifies multi-tool usage patterns
